    ...
    try:
        ...
        for chunk in response:
            if hasattr(chunk.choices[0].delta, "content"):
                yield chunk.choices[0].delta.content.encode('utf-8')  # ✅ encode to bytes
    except Exception as e:
        yield f"[ERROR] {str(e)}".encode('utf-8')  # ✅ also encode error
...
def handle_query():
    try:
        ...
        if guidedRoute == 'products':
            ...
            save_chat_log(messages)
            return Response(stream_with_context(stream_openai_response(messages)), mimetype='text/plain')
        ...
        # 3️⃣ Route: chitchat
            ...
            save_chat_log(chat_messages)
            return Response(stream_with_context(stream_openai_response(chat_messages)), mimetype='text/plain')
